...
def test_cpu_workload_keys_exist() -> None:
    required_keys = ["cpu-smoke", "cpu-balanced", "cpu-decode-heavy", "cpu-prefill-heavy"]
    for key in required_keys:
        wl = runtime_config.get_workload(key)
        assert "data" in wl, f"{key}: missing 'data'"
        assert "rates" in wl, f"{key}: missing 'rates'"
        assert "max_seconds" in wl, f"{key}: missing 'max_seconds'"
        assert isinstance(wl["rates"], list) and wl["rates"], f"{key}: 'rates' must be non-empty list"
    print(f"  cpu workload keys exist: {required_keys}  OK")
