...
Use the `cpu-balanced` preset (RHAIIS flavor, llama31-8b-w8a8-cpu, KV=24 GiB) or
`vanilla-cpu-balanced` for the upstream vLLM image. The preset sets model, workload,
flavor, 64 GiB memory, and `VLLM_CPU_KVCACHE_SPACE=24`; the
overrides below raise CPU cores and memory for a production-sized node.
...
```
/test fournos rhaiis cpu-balanced
/pipeline forge-full
/cluster zeus
...
Presets from `args` are applied via `project.args` → files under `presets.d/`.
...
Presets in `presets.d/` (models, workloads, engines, ci_tests, etc.) provide shortcuts:
