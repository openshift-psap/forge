...
Before running `cpu-balanced` or a Fournos 8B job, resize or recreate the
PVC at 200 Gi:
...
> **Note**: TinyLlama's `max-model-len` is 2048; do not use `cpu-decode-heavy`
> or `cpu-prefill-heavy` with it (prompt_tokens exceed the context limit for
> some shapes). Use `qwen3-0-6b-cpu` or another model without a 2048 cap.

```bash
# TinyLlama — balanced workload only (max-model-len: 2048)
python -m projects.rhaiis.orchestration.cli concurrent-load \
  --preset cpu-vanilla \
  --models tinyllama-cpu \
  --cpu-requests 8,16 \
  --workloads cpu-balanced \
  --namespace forge-rhaiis \
  --continue-on-error

# Qwen3-0.6B — all three ISL/OSL shapes
python -m projects.rhaiis.orchestration.cli concurrent-load \
  --preset cpu-vanilla \
  --models qwen3-0-6b-cpu \
  --cpu-requests 8,16 \
  --workloads cpu-balanced,cpu-decode-heavy,cpu-prefill-heavy \
  --namespace forge-rhaiis \
  --continue-on-error
```
...
```bash
# cpu-balanced preset pins model (llama31-8b-w8a8-cpu), flavor (rhaiis),
...
`--preset cpu-balanced` or `--preset vanilla-cpu-balanced` sets the
...
| `--workloads` | `cpu-balanced` | Safe with all models; add decode/prefill-heavy explicitly for capable models |
...
| `cpu-balanced` | 512 | 512 | 600s, rates 1-32 |
| `cpu-decode-heavy` | 256 | 2048 | 600s, rates 1-32 |
| `cpu-prefill-heavy` | 2048 | 256 | 600s, rates 1-16 |
...
CPU-specific presets live in `presets.d/ci_tests.yaml` (smoke/balanced jobs) and
`presets.d/engines.yaml` (`cpu` / `cpu-vanilla` accelerator presets). Combine with
model/workload presets as needed:
...
| `cpu-balanced` | rhaiis | `llama31-8b-w8a8-cpu` | `cpu-balanced` |
| `vanilla-cpu-balanced` | vanilla | `llama31-8b-w8a8-cpu` | `cpu-balanced` |
